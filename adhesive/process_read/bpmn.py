...
def process_node(result: Process, node) -> None:
...
def process_boundary_event(result: Process, node) -> None:
    node_ns, node_name = parse_tag(node)
...
def process_edge(result: Process, node) -> None:
    node_ns, node_name = parse_tag(node)
...
def process_lane_set(process: Process, result_set_node) -> None:
...
def process_lane(process: Process, lane_node) -> None:
    ...
    lane = Lane(
        id=lane_node.get("id"), name=lane_node.get("name"), parent_process=process
    )
...
def process_node_task(p: Process, xml_node) -> None:
    ...
    task = Task(parent_process=p, id=xml_node.get("id"), name=node_name)
    ...
    for node in list(boundary_event_node):
        ...
        if node_name == "errorEventDefinition":
            boundary_task = ErrorBoundaryEvent(
                parent_process=p, id=boundary_event_node.get("id"), name=task_name
            )
...
def read_timer_event_definition(
    *, process, boundary_event_node, timer_event_definition_node, task_name
):
    ...
    for node in list(timer_event_definition_node):
        ...
        if "timeCycle" == node_name:
            ...
            raise Exception(
                f"Invalid node <{node_name}>, only <timeCycle>, <timeDuration> "
...
def process_node_start_event(p: Process, xml_node) -> None:
    ...
    if message_event_node is not None:
        message_event = MessageEvent(
            parent_process=p, id=xml_node.get("id"), name=node_name
        )
...
def process_exclusive_gateway(p: Process, xml_node) -> None:
    ...
    node_name = normalize_name(xml_node.get("name"))
    task = ExclusiveGateway(parent_process=p, id=xml_node.get("id"), name=node_name)
...
def process_parallel_gateway(p: Process, xml_node) -> None:
    """ Create an end event from the process """
    node_name = normalize_name(xml_node.get("name"))
    task = ParallelGateway(parent_process=p, id=xml_node.get("id"), name=node_name)
...
def process_complex_gateway(p: Process, xml_node) -> None:
    ...
    task = ComplexGateway(parent_process=p, id=xml_node.get("id"), name=node_name)
...
def process_potential_loop(task: PT, xml_node) -> PT:
    ...
    if loop_node and multi_instance_loop:
        raise Exception(
            f"Both standard loop and multi instance loop were present on {xml_node}"
        )

    if loop_node:
        loop_expression = find_node(loop_node, "loopCondition")
        task.loop = Loop(
            loop_expression=textwrap.dedent(loop_expression.text), parallel=True
        )
        ...
        task.loop = Loop(
            loop_expression=textwrap.dedent(loop_expression.text),
